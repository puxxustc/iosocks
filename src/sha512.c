...
static inline uint64_t sha512_be64dec(const void *pp)
{
...
static inline void sha512_be64enc(void *pp, uint64_t x)
{
...
static void be64enc_vect(uint8_t *dst, const uint64_t *src, size_t len)
{
	for (size_t i = 0; i < len / 8; i++)
	{
		sha512_be64enc(dst + i * 8, src[i]);
	}
}

static void be64dec_vect(uint64_t *dst, const uint8_t *src, size_t len)
{
	for (size_t i = 0; i < len / 8; i++)
	{
		dst[i] = sha512_be64dec(src + i * 8);
	}
}
...
static void sha512_update(sha512_state *state, const uint8_t *in, size_t len)
{
	uint64_t bitlen[2];
	uint64_t r;
	...
	memcpy(state->buf, src, len);
}

static void sha512_pad(sha512_state *state)
{
	static uint8_t pad[128] =
	{
	...
	sha512_update(state, pad, (size_t)plen);

	sha512_update(state, len, 16);
}

static void sha512_final(sha512_state *state, void *digest)
{
	sha512_pad(state);
	be64enc_vect(digest, state->state, 64);
	bzero((void *)state, sizeof(*state));
}

void sha512(void *digest, const void *in, size_t len)
{
	sha512_state state;

	sha512_init(&state);
	sha512_update(&state, in, len);
